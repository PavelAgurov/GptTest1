...
class LLMManager():
    ...
    def refine_text(self, text : str) -> str:
        ...
        self.report_status('Split documents for refining...')
        split_docs = self.text_splitter.split_documents([Document(page_content = text)])
        self.report_status('')

        self.report_status('Request LLM for summary...')
        refine_result = RefineChain(self.llm_summary).refine(split_docs)
        summary = ""
    ...
    def translate_text(self, text : str) -> TranslationResult:
        ...
        try:
            translated_text_json = json.loads(get_fixed_json(translated_text))
        ...
        for i, p in enumerate(paragraph_list):
            for j, topic_def in enumerate(topic_chunks):
                ...
                try:
                    ...
                    extracted_score_json = json.loads(get_fixed_json(extracted_score))
                    self.report_status('')
